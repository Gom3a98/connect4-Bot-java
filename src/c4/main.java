...
public static int getRandom(Vector  array) {
    ...
    return array.indexOf(rnd);
}
public static int get_next_open_row(char [][]board ,int col)
{
    ...
    public static Vector<Integer> get_valid_locations(char[][] board) {
        Vector<Integer> valid_locations = new Vector<>();
        for (int i = 0; i < COLUMN_COUNT; i++) {
            if (board[ROW_COUNT-2][i] == ' '&&board[ROW_COUNT-2][i]!='x'&&board[ROW_COUNT-2][i]!='o') {
...
public static int score_position(char [][]board ,char p)
{
    ...
    public static int [] minimax(char[][] board, int depth, int alpha, int beta, boolean maximizingPlayer) {
        ...
        boolean is_terminal = test(board, AI_PIECE) || test(board, PLAYER_PIECE) || valid_locations.size() == 0;
